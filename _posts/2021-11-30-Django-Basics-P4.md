...
After getting familiar to the folder structure of Django framework, we'll create our first view in an app. The basics of creating and mapping a view with a url will be cleared by the end of this part.

## Creating Views

> Views are the functions written in python as a logic control unit of the web server

To create a view or typically like function, we need to write a function in the `views.py` file inside of the application folder. The function name can be anything but should be a sensible name as far as its usability is concerned. Let's take a basic example of sending a http response of "Hello World".
...
Yes, we are simply returning a HTTP Response right now, but rendering Templates/HTML Documents is quite similar and easy to grasp in Django. So, this is a view or a piece of logic but there is a piece missing in this. Where should this function be used? Of course a URL i.e a path to a web server.
...
The path function in the urlpatterns takes in at least 2 parameters, i.e. the url pattern and the view or any other function that can be related to the web server. 
...
The URL Path is the pattern or a literally the path which you use in the Browser's search bar. This can be static i.e. some hard-coded text like `home/`, `user/`, `post/home/`, etc. and we can also have dynamic URLs like `post/<pk:id>/`, `user/<str:name>/`, etc. here the characters `<pk:id>` and `<str:name>` will be replaced by the actual id(integer/primary key) or the name(String) itself. 

This is used in an actual web-application, where there might be user profile which needs the unique user-id to render it specifically for that user. The User-Profile is just an example, it can anything like post, mails, products, any form of a content driven application. 

### View

The view or the function is the name of the function that will be attached to that url path. That means once the user visits that url, the function is literally called. **View is just a fancy word for a function(or any logic basically).** There is a lot to be covered when it comes to `View` as there are lot of ways to create it, there are two types of views, how to use them for various use-cases that can be learnt along the way because it is a topic where the crust of Django exist.  

We'll learn to create different implementations and structure our views, for time-being just consider them as the unit where every operation on the web can be performed. We can create other standalone functions in python to work with the views to make it a bit structures and readable.

### URL Name

This is a optional parameter to the path function as we do not mandatorily need to give the url map a name. This can be really useful in multi-page application websites where you need to link one page to another and that becomes a lot easier with the url name. We do not need this right now, we'll touch it when we'll see the Django Templating Language. 

## Example Views

Let's create some examples to understand the working of Views and URLs. We'll create a dynamic URLs and integrate Python module in the views to get familiarized with the concept.
...
This view or function takes additional argument called `name` and in response it just says `Welcome, name` where name can be any string. Now after creating the view, we need to map the view to a url pattern, We'll add a path for this greet function. 
...
Now, how is this working? We see the view first. The function takes two parameters one is most common the request which stores the meta-data about the request, the other parameter is name which we will be using to respond to the server dynamically. The name variable is used in the string with the HttpResponse function to return a simple string.
